...
@lexer::members {
    ...
    void handleUnrecognizedChar(const std::string& text, int line) {
        //std::ofstream errorFile("error.txt", std::ios::app);
        errorFile << "Error at line " << line << ": Unrecognized character '" << text << "'" << std::endl << std::endl;
errorCount++;
        errorFile.flush();
        parserLogFile << "Error at line " << line << ": Unrecognized character '" << text << "'" << std::endl << std::endl;
        parserLogFile.flush();
...
CONST_FLOAT
    ...
    ;
    // ------------------------------
// 6) Error handling for unrecognized characters
// ------------------------------

// Error handling 
// Add this to your C8086Lexer.g4 file:
